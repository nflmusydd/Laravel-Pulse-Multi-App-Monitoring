...
<x-pulse::card :cols="$cols" :rows="$rows" :class="$class">
    ...
    >
        ...
        <x-slot:actions>
            ...
            @php
                $appOptions = $slowOutgoingRequests->pluck('app_name')->filter()->unique()->sort();
            ...
            <x-pulse::select
                ...
                id="select-slow-outgoing-requests-app-name"
                label="Filter App"
                :options="$appOptions"
                @change="loading = true"
            />
        ...
        @else
            <x-pulse::table>
                ...
                <tbody>
                    ...
                    @foreach ($slowOutgoingRequests->take(100)->filter(fn($query) => empty($appName) || $query->app_name === $appName)->take(100)  as $request)
