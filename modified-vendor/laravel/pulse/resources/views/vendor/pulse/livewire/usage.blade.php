<x-pulse::card :cols="$cols" :rows="$rows" :class="$class">
    <x-pulse::card-header
        :name="match ($this->type) {
            'requests' => 'Top 12 Users Making Requests',
            'slow_requests' => 'Top 12 Users Experiencing Slow Endpoints',
            'jobs' => 'Top 12 Users Dispatching Jobs',
    ...
    >
        ...
        <x-slot:actions>
            ...
            @php
                ...
                $appOptions = ['' => 'All'] + $appOptions->mapWithKeys(fn($name) => [$name => $name])->toArray();
            @endphp

            <x-pulse::select
                wire:model.live="appName"
                id="select-usage-app-name"
                label="Filter App"
                :options="$appOptions"
                class="flex-1"
                @change="loading = true"
