    ...
    >
        ...
        <x-slot:actions>
            <div class="flex justify-between w-full flex-wrap">
                ...
                <div class="flex">
                    @php
                        $appOptions = $queues->keys()->unique()->sort();

                        if ($filter && !$appOptions->contains($filter)) {
                            $appOptions->push($filter);
                        }

                        $appOptions = [null => 'All'] + $appOptions->mapWithKeys(fn($name) => [$name => $name])->toArray();
                        // $appOptions = ['all' => 'All'] + $appOptions->mapWithKeys(fn($name) => [$name => $name])->toArray();
                        // $appOptions = [null => 'All'] + $queues->keys()->unique()->sort()->mapWithKeys(fn($name) => [$name => $name])->toArray();
                        // $appOptions = ['' => 'All'] + $queues->keys()->unique()->sort()->mapWithKeys(fn($name) => [$name => $name])->toArray();
                    @endphp

                    <x-pulse::select
                        wire:model.live="filter"
                        id="select-queues-app-filter"
                        label="Filter App"
                        :options="$appOptions"
                        @change="loading = true"
                    />
    ...
    <x-pulse::scroll :expand="$expand" wire:poll.5s="">
        ...
        @else
            <div class="grid gap-3 mx-px mb-px">
                ...
                @foreach ($queues as $app => $queueGroup)
                    <div class="mb-6">

                        <div class="grid gap-3 mx-px mb-px mt-2">
                                @if (!$filter || $filter === $app)
                                    <div class="mb-6">
                                        @php
                                            $showApp = empty($app) ? 'Unknown Apps' : $app;
                                        @endphp
                                        ...
                                        <div class="grid gap-3 mx-px mb-px mt-2">
                                            @foreach ($queueGroup as $queue => $readings)
                                                <div wire:key="{{ $app.'-'.$queue }}">
                                                    {{-- <h3 class="font-bold text-gray-700 dark:text-gray-300">
                                                        @if ($showConnection)
                                                            {{ $queue }}
                                                        @else
                                                    ...
                                                    <div class="mt-3 relative">
                                                        ...
                                                        >
                                                            <canvas
                                                                x-ref="canvas"
                                                                class="ring-1 ring-gray-900/5 dark:ring-gray-100/10 bg-gray-50 dark:bg-gray-800 rounded-md shadow-sm"
                                                            ></canvas>
                                                        </div>
                                                    </div>
                                                </div>
...
Alpine.data('queueChart', (config) => ({
    init() {
        ...
        //     const [app, queue] = config.queue.split(".");
        //     if (!queues[app] || !queues[app][queue]) {
        //         chart.destroy();
        //         chart = undefined;
        //         return;
        //     }
        ...
        Livewire.on('queues-chart-update', ({ queues }) => {
            ...
            let data = queues[app][queue];
            chart.data.labels = this.labels(data);
            chart.options.scales.y.max = this.highest(data);
            chart.data.datasets[0].data = this.scale(data.queued);
            chart.data.datasets[1].data = this.scale(data.processing);
            chart.data.datasets[2].data = this.scale(data.released);
            chart.data.datasets[3].data = this.scale(data.processed);
            chart.data.datasets[4].data = this.scale(data.failed);
            chart.update();
        })
    },
    labels(readings) {
