...
use function Laravel\Pulse\Storage\graph;
            ...
            return $this->values('system')
                ->map(function ($system, $slug) use ($graphs) {
                    ...
                    return (object) [
                        ...
                        'name' => ($system->ip_hostname ?? '(IP)') . ' - ' . ($system->container_name ?? '(Container Name)'),
                        'cpu_current' => (int) $values->cpu,
                        'cpu' => $graphs->get($slug)?->get('cpu') ?? collect(), // beberapa nilai null
                        'memory_current' => (int) $values->memory_used,
                        'memory_total' => (int) $values->memory_total,
                        'memory' => $graphs->get($slug)?->get('memory') ?? collect(), // beberapa nilai null
