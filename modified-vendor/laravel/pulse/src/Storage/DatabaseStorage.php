...
class DatabaseStorage implements Storage
{
    ...
    public function store(Collection $items): void
    {
        ...
        $this->connection()->transaction(function () use ($entryChunks, $countChunks, $minimumChunks, $maximumChunks, $sumChunks, $averageChunks, $valueChunks) {
            ...
            // ->table('pulse_entries')
            // // ->insert($chunk->all())); // bawaan
            // ->insert(
            //     $chunk->map(fn ($row) => array_merge($row, [
            //         'app_name' => config('app.name'),  //tidak pakai $appName
            //     ]))->all()
            // ));
                        
            $entryChunks->each(function ($chunk) {
                $appName = config('app.name');
                
                $rows = collect($chunk->all())->map(function ($row) use ($appName) {
                ...
                $this->connection()->table('pulse_entries')->insert($rows);
            });
            ...
            //     //     ]))->all(),
            //     //     ['type', 'key_hash', 'app_name'], // conflict keys
            //     //     ['timestamp', 'value']
            //     // )
            // );

            $valueChunks->each(function ($chunk) {
                $appName = config('app.name');
                $ip = config('app.ip') ?? trim(exec("hostname -i"));
                $containerName = config('app.container_name') ?? trim(exec('hostname'));

                //tambah app_name, ip_hostname, container_name terbaru
                $rows = collect($chunk->all())->map(function ($row) use ($appName, $ip, $containerName) {
                    if (!is_array($row)) {
                        $row = (array) $row;
                    }
                ...
                $rowsdd = $rows;
                $rows->each(function ($row) use ($appName, $ip, $containerName, $rowsdd) {
                    ...
                    try {
                        if (!$existing){
                            ...
                            if ($existing && $existing->ip_hostname == $row['ip_hostname'] && $existing->app_name == $row['app_name'] && $row['type'] == 'system'){
                            // [IP Server saat ini] = ip && [appname server saat ini] = app_name && type = 'system'
                            $this->connection()
                                ->table('pulse_values')
                                ...
                                if ($existing && $existing->container_name == $row['container_name'] && $row['type'] == 'system' && (empty($existing->ip_hostname) || empty($existing->app_name))){
                                    //  [container name saat ini] = container_name && type = 'system' && (ip = blank || app_name = blank)) => update ip dan app_name saat ini
                                    $this->connection()
                                        ->table('pulse_values')
                                        ->where('id', $existing->id)
                                        ->update([
                                            'ip_hostname' => $row['ip_hostname'],
                        ...
                        // Log::error('Pulse DB insert/update error', [
                        //     'message' => $e->getMessage(),
                        //     'row' => $row,
                        // ]);
                        dd( $e->getMessage());  //lihat error waktu pulse:check
                    }
                });
                // $this->connection() 
                //     ->table('pulse_values')
                //     ->upsert($rows->all(), ['type','key_hash'], []);
            });
    ...
    {
        // Log::info('Pulse Database Storage function upsertCount:  ' . __LINE__);
        $appName = config('app.name');
        // Log::debug($appName);
 
        // ensure app_name on each value row
        $values = array_map(function ($v) use ($appName) {
            $row = (array) $v;
            // $row['app_name'] = $row['app_name'] ?? $appName;
            $row['app_name'] = $appName;
            return $row;
        }, $values);
        
        // include app_name in conflict keys
        $conflictKeys = ['bucket', 'period', 'type', 'aggregate', 'key_hash', 'app_name'];
        
        return $this->connection()->table('pulse_aggregates')->upsert(
            $values,
            $conflictKeys,
            // keep the original $updates array per aggregate type (example below)
            [
        ...
        //     // ['bucket', 'period', 'type', 'aggregate', 'key_hash', 'app_name'],
        //     [
        //         'value' => match ($driver = $this->connection()->getDriverName()) {
        //             'mariadb', 'mysql' => new Expression('`value` + values(`value`)'),
        //             'pgsql', 'sqlite' => new Expression(<<<SQL
    ...
    {
        // Log::info('Pulse Database Storage function upsertMin:  ' . __LINE__);
        $appName = config('app.name');
        ...
        return $this->connection()->table('pulse_aggregates')->upsert(
            ...
            [
                'value' => match ($driver = $this->connection()->getDriverName()) {
                    ...
                    'sqlite' => new Expression('min(value, excluded.value)'),
                    default => new Expression('least("value", values("value"))'),
                },
            ]
        );
    ...
    {
        // Log::info('Pulse Database Storage function upsertMax:  ' . __LINE__);
        $appName = config('app.name');
    ...
    {
        // Log::info('Pulse Database Storage upsertSum:  ' . __LINE__);
        $appName = config('app.name');
    ...
    {
        // Log::info('Pulse Database Storage function upsertAvg:  ' . __LINE__);
        $appName = config('app.name');
        ...
        //             'value' => new Expression('(`value` * `count` + (values(`value`) * values(`count`))) / (`count` + values(`count`))'),
        //             'count' => new Expression('`count` + values(`count`)'),
        //             // 'app_name' => new Expression('values(`app_name`)'),  //tambah app_name
        //         ],
        //         'pgsql', 'sqlite' => [
        //             'value' => new Expression(<<<SQL
        //                 ({$this->wrap('pulse_aggregates.value')} * {$this->wrap('pulse_aggregates.count')} + ("excluded"."value" * "excluded"."count")) / ({$this->wrap('pulse_aggregates.count')} + "excluded"."count")
        ...
        //                 {$this->wrap('pulse_aggregates.count')} + "excluded"."count"
        //                 SQL),
        //             // 'app_name' => new Expression('"excluded"."app_name"'),  //tambah app_name
        //         ],
        //         default => throw new RuntimeException("Unsupported database driver [{$driver}]"),
        //     }
        // );
    ...
    public function values(string $type, ?array $keys = null): Collection
    {
        ...
        return $this->connection()
            ->table('pulse_values')
            ->select('timestamp', 'key', 'value', 'ip_hostname', 'container_name')
            ->where('type', $type)
    ...
    public function graph(array $types, string $aggregate, CarbonInterval $interval): Collection
    {
        ...
        $raw = $this->connection()->table('pulse_aggregates')
            ->select(['bucket', 'type', 'key', 'value', 'app_name'])
            ->whereIn('type', $types)
        ...
        // Kelompokkan apakah graph untuk server atau queue
        $serverMetric = ['cpu', 'memory'];
        $queueMetric = ['queued', 'processing', 'processed', 'released', 'failed'];

        //grafik server masih pakai default pulse
        if ($types === $serverMetric) {
            $results = $this->connection()->table('pulse_aggregates')
        ...
        } elseif ($types === $queueMetric){ //grafik queue custom sendiri
            $results = $raw
                ->groupBy('app_name') // Level 1: app1, app2
                ->map(function ($readingsByApp) use ($structure, $padding) {
                    return $readingsByApp
                        ->groupBy('key') // Level 2: database:default
                        ->sortKeys()
                        ->map(function ($readingsByKey) use ($structure, $padding) {
                            return $structure->merge( // pastikan semua type ada
                                $readingsByKey
                                    ->groupBy('type') // Level 3: queued, processing, released, ...
                                    ->map(function ($readingsByType) use ($padding) {
                                        return $padding->merge( // pastikan semua timestamp ada
                                            $readingsByType->mapWithKeys(function ($reading) {
                                                return [
                                                    CarbonImmutable::createFromTimestamp($reading->bucket)->toDateTimeString() => $reading->value,
                                                ];
                                            })
                                        );
                                    })
            ...
            throw new InvalidArgumentException("Jenis metric tidak dikenali: " . $types);
            // $results = $this->connection()->table('pulse_aggregates')
            //                 ->select(['bucket', 'type', 'key', 'value'])
            //                 ->whereIn('type', $types)
            //                 ->where('aggregate', $aggregate)
        ...
        return $this->connection()
            ...
            ->select([
                'key' => fn (Builder $query) => $query
                    ...
                    ->limit(1),
                ...$aggregates,
                'app_name' => fn (Builder $query) => $query
                    ->select('app_name')
                    ->from('pulse_entries', as: 'keys')  // Ambil dari tabel pulse_entries
                    ->whereColumn('keys.key_hash', 'aggregated.key_hash')
                    ->limit(1),
                ...$aggregates,
            ])
            ->fromSub(function (Builder $query) use ($type, $aggregates, $interval, $orderBy, $direction, $limit) {
                ...
                $query->select('key_hash', 'app_name');
